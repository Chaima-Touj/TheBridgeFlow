 ...
 *
 * Usage :
 *   node server/scripts/drive/authenticate.js drive1
 *   node server/scripts/drive/authenticate.js drive2
 *
 ...
 * token, et le sauvegarde dans server/token-<compte>.json (gitignore).
 */
...
if (!ACCOUNTS[accountKey]) {
  console.error("Usage : node server/scripts/drive/authenticate.js <drive1|drive2>");
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`[${accountKey}] ✅ Token sauvegardé dans server/token-${accountKey}.json`);
}
